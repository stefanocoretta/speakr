...
The package speakr allows the user to run a Praat script from within R, optionally passing paratmeters to the script if a form is used.
...
formants <- praat_run(script, unit, window, capture = TRUE) %>%
  read_csv()
attr(formants, "args") <- list(unit = unit, window = window)
```

Let's have a look at `formants`.
